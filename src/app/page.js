...
export default function Landing2() {
  ...
  const [showPlansInDialog, setShowPlansInDialog] = useState(false) // false = Form 1 (no plans), true = Form 2 (with plans)
  const [dialogNote, setDialogNote] = useState('')
  const [formData, setFormData] = useState({ name: '', email: '', password: '', plan: 'basic' })
  const [loadingSubmit, setLoadingSubmit] = useState(false)
  const [submitError, setSubmitError] = useState('')

  // Open dialog helper: withPlans = true shows plans (Form 2), false shows simple form (Form 1)
  ...
  // Submit handler: register user then redirect to dashboard (subscription handled in dashboard)
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Close dialog and redirect to dashboard; actual subscription flow happens in dashboard
      ...
      >
        <div className="container mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
            {/* Get Started opens simple form (Form 1) */}
      ...
      <motion.section style={{ opacity, scale }} className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              {/* Start Preparing opens simple form (Form 1) */}
              <Button
                size="lg"
              ...
              {/* Start Preparing Today opens form with plans (Form 2) */}
              <Button
                size="lg"
                variant="outline"
                ...
                <Card className={`relative overflow-hidden h-full ${plan.popular ? 'bg-white border-white' : 'bg-[#141414] border-[#2a2a2a]'}`}>
                  ...
                  <CardFooter className="pt-6">
                    <Button
                      ...
                      size="lg"
                      onClick={() => {
                        // Open dialog WITH plans and preselect this plan (Form 2)
