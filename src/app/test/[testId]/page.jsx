...
export default function TestPage() {
  ...
  // handleNext: special-case listening piece transitions (no alert when moving piece->piece)
  const handleNext = async () => {
    ...
    if (currentChapter.type === 'listening') {
      ...
      // if not last piece, just advance to next piece silently
      ...
      // if last piece in this listening chapter, now check unanswered across chapter before moving to next chapter
      ...
      // move to next chapter (intro logic handled by effect)
      setCurrentChapterIndex(ci => ci + 1);
      setCurrentPieceIndex(0);
      return;
    }

    // reading flow unchanged: move piece->piece with checks only at end of chapter
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <main>
        ...
        {/* Listening: left = piece (audio/text), right = questions */}
        {currentChapter?.type === 'listening' && currentPiece && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* LEFT: full piece content */}
            <div className="bg-white rounded-lg p-6 shadow-sm max-h-[70vh] overflow-y-auto text-left">
              ...
              {/* If questions include base_text with underlines, show examples beneath piece */}
