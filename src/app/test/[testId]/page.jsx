...
export default function TestPage() {
    ...
    try {
      ...
      for (const ch of chaptersData || []) {
        if (ch.type === 'listening') {
          ...
          // parse passage into paragraphs (if passage contains "1. " markers we split)
          const normalizedPieces = (pieces || []).map(p => {
            const piece = { ...p };
            if (piece.passage && typeof piece.passage === 'string') {
              // try to split on double newlines and detect "N. " prefix
  ...
  const handleNext = async () => {
    ...
    if (currentChapter.type === 'listening') {
      ...
      if (currentPieceIndex < last) { setCurrentPieceIndex(i => i + 1); setPhase('questions'); return; }
      else {
        ...
        setCurrentChapterIndex(ci => ci + 1); setCurrentPieceIndex(0); return;
      }
    }
    ...
    if (currentChapter.type === 'grammar') {
      ...
      if (currentPieceIndex < last) { setCurrentPieceIndex(i => i + 1); return; }
        ...
        await finalizeScoresAndWrongs(); setShowResult(true); return;
      }
    }
  };
  ...
  // underlined renderer & parsing helper
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <main>
        ...
        {currentChapter?.type === 'listening' && (
          <div>
            {phase === 'intro' && (<div className="max-w-3xl mx-auto mb-6"><div className="bg-white rounded-lg p-8 text-center shadow-sm"><Volume2 className="w-16 h-16 text-blue-600 mx-auto mb-4" /><h2 className="text-2xl font-semibold mb-2">قسم الاستماع</h2><p className="text-slate-600 mb-4">اضغط ابدأ للاستماع ثم الإجابة عن أسئلة المقطع.</p><div className="flex justify-center gap-3"><Button onClick={() => setPhase('questions')} className="bg-blue-600 text-white">ابدأ</Button><Button onClick={() => setPhase('questions')} variant="outline">تخطي</Button></div></div></div>)}

            {phase === 'questions' && currentPiece && (
              <div className="max-w-4xl mx-auto space-y-6">
                <div className="bg-white rounded-lg p-6 shadow-sm"><audio controls className="w-full" preload="none"><source src={currentPiece.audio_url} type="audio/mpeg" />متصفحك لا يدعم عناصر الصوت.</audio></div>

                <div className="flex flex-wrap gap-2">{currentPiece.listening_questions.map((q, qi) => { const answered = !!answeredMap[q.id]; const marked = !!markedMap[q.id]; const cls = answered ? 'bg-green-600 text-white' : 'bg-slate-200 text-slate-700'; return (<button key={q.id} onClick={() => goToQuestionInCurrent(q.id)} className={`px-2 py-1 rounded ${cls}`}>{qi + 1}{marked && <span className="ml-2 text-xs">★</span>}</button>); })}</div>
                ...
                {currentPiece.listening_questions.map((q, i) => (
                  <div key={q.id} id={`q-${q.id}`} className="bg-white rounded-lg p-6 shadow-sm">
                    ...
                    <div className="space-y-2">{q.options?.map((opt, oi) => (<label key={oi} className={`flex items-center p-4 border-2 rounded-md cursor-pointer ${answers[q.id] === opt ? 'border-blue-600' : 'border-slate-200'}`}><input type="radio" name={`q-${q.id}`} value={opt} onChange={() => handleSelect(q.id, opt)} checked={answers[q.id] === opt} className="w-4 h-4 text-blue-600 mr-3" /><span className="font-medium mr-2">{String.fromCharCode(65 + oi)}.</span><span>{opt}</span></label>))}</div>
                  </div>
                ))}

                <div className="flex justify-between mt-4"><Button onClick={handlePrev} variant="outline"><ArrowLeft className="w-4 h-4 ml-2" /> السابق</Button><Button onClick={handleNext} className="bg-blue-600 text-white">التالي <ChevronRight className="w-4 h-4 ml-2" /></Button></div>
              </div>
            )}
          </div>
        )}
        ...
        {currentChapter?.type === 'reading' && currentPiece && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg p-6 shadow-sm max-h-[70vh] overflow-y-auto text-left">
              ...
              {/* if passage_paragraphs exists show numbered paragraphs (num bold) else show passage text */}
                ...
                {currentPiece.reading_questions.map((q, idx) => {
                  ...
                  return (
                    <button key={q.id} onClick={() => goToQuestionInCurrent(q.id)} className={`px-2 py-1 rounded ${cls}`}>
                      {idx + 1}{marked && <span className="ml-2 text-xs">★</span>}
                    </button>
                  );
                })}
              </div>
