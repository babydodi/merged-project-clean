...
export default function Landing2() {
  ...
  const features = [
    { icon: BookOpen, title: 'Real Test Simulator', description: 'Practice with authentic test conditions and interface that mirrors the actual STEP exam', delay: 0.2 },
    { icon: Brain, title: 'Detailed Explanations', description: 'Understand every question with comprehensive explanations that help you learn from mistakes', delay: 0.4 },
    { icon: Trophy, title: 'Track Your Progress', description: 'Monitor your improvement with detailed analytics and personalized performance insights', delay: 0.6 },
  ]
  ...
  const plans = [
    ...
    },
    {
      ...
      features: ['Full test simulator access', 'Detailed explanations for every question', 'Advanced analytics & insights', 'Priority support', 'Unlimited practice tests'],
      popular: true,
      delay: 0.4,
    },
  ]
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setLoading(true)
    // هنا مجرد محاكاة إرسال؛ عدّل حسب تكاملك مع Supabase إذا تريد تسجيل حقيقي
    try {
      // لو عندك supabase auth مفعل:
      // await supabase.auth.signUp({ email: formData.email, password: formData.password, options: { data: { full_name: formData.name, plan: formData.plan } } })
      // الآن ننتقل للداشبورد كمثال
      ...
      <motion.section style={{ opacity, scale }} className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="text-5xl md:text-7xl font-bold mb-6 leading-tight text-white">
              Master the STEP English Test
            </motion.h1>
      ...
      <section className="py-32 relative border-t border-[#1a1a1a]">
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {features.map((f, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: f.delay }} viewport={{ once: true }} whileHover={{ y: -10, transition: { duration: 0.3 } }}>
                <Card className="bg-[#141414] border-[#2a2a2a] hover:border-white transition-all duration-300 h-full">
                  <CardHeader>
                    <motion.div whileHover={{ rotate: 360, scale: 1.1 }} transition={{ duration: 0.6 }} className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center mb-4">
                      {f.icon && <f.icon className="w-8 h-8 text-black" />}
                    </motion.div>
                    <CardTitle className="text-2xl text-white">{f.title}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <CardDescription className="text-gray-400 text-base leading-relaxed">{f.description}</CardDescription>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
              ...
              <motion.div key={plan.name} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: plan.delay }} viewport={{ once: true }} whileHover={{ scale: 1.05, transition: { duration: 0.3 } }}>
                <Card className={`relative overflow-hidden h-full ${plan.popular ? 'bg-white border-white' : 'bg-[#141414] border-[#2a2a2a]'}`}>
                    ...
                    <ul className="space-y-4">
                      {plan.features.map((f, idx) => (
                        <motion.li key={idx} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: plan.delay + 0.1 * idx }} viewport={{ once: true }} className="flex items-start gap-3">
                          ...
                          <span className={plan.popular ? 'text-gray-700' : 'text-gray-400'}>{f}</span>
                        </motion.li>
                      ))}
                    </ul>
                  </CardContent>
                  ...
                  <CardFooter className="pt-6">
                    <Button className={`w-full transition-all transform hover:scale-105 ${plan.popular ? 'bg-black text-white hover:bg-gray-900' : 'bg-white text-black hover:bg-gray-200'}`} size="lg" onClick={() => { setFormData({ ...formData, plan: plan.name.toLowerCase() }); setIsSignUpOpen(true); }}>
            ...
            <div className="relative z-10">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} viewport={{ once: true }}>
                <Button size="lg" onClick={() => setIsSignUpOpen(true)} className="bg-black text-white hover:bg-gray-900 transition-all transform hover:scale-105 text-lg px-10 py-6">
                  Start Preparing Today <Zap className="ml-2 w-5 h-5" />
                </Button>
              </motion.div>
            </div>
          </motion.div>
        ...
        <DialogContent className="bg-[#141414] border-[#2a2a2a] text-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="Enter your name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className="bg-[#0a0a0a] border-[#2a2a2a] text-white focus:border-white" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="email" className="text-white">Email Address</Label>
              <Input id="email" type="email" placeholder="Enter your email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required className="bg-[#0a0a0a] border-[#2a2a2a] text-white focus:border-white" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                {plans.map((p) => (
                  <button key={p.name} type="button" onClick={() => setFormData({ ...formData, plan: p.name.toLowerCase() })} className={`flex-1 p-4 rounded-lg border-2 transition-all ${formData.plan === p.name.toLowerCase() ? 'border-white bg-white/10' : 'border-[#2a2a2a] hover:border-[#3a3a3a]'}`}>
